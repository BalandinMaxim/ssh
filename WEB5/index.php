...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE['save'])) {
    ...
    if (!empty($_COOKIE['pass'])) {
      $messages[] = sprintf('<a href="log.php">Войти</a> с логином <strong>%s</strong>
  ...
  if (!$error && !empty($_COOKIE[session_name()]) && !empty($_SESSION['login'])) {
    ...
    try{
      $get=$db->prepare("SELECT * FROM FORMS_CHAR WHERE id=?");
      ...
      $get2=$db->prepare("SELECT name FROM SUPER_CHAR WHERE per_id=?");
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
  ...
  if(!empty($_POST['logout'])){
    ...
    if (!empty($_COOKIE[session_name()]) && !empty($_SESSION['login']) and !$errors) {
      $id=$_SESSION['uid'];
      $upd=$db->prepare("UPDATE FORMS_CHAR SET name=:name, email=:email, year=:byear, pol=:pol, limbs=:limbs, bio=:bio WHERE id=:id");
      $cols=array(
        ':name'=>$name,
        ':email'=>$email,
        ':byear'=>$year,
      ...
      $upd->bindParam(':id',$id);
      $upd->execute();
      $del=$db->prepare("DELETE FROM SUPER_CHAR WHERE per_id=?");
      $del->execute(array($id));
      $upd1=$db->prepare("INSERT INTO SUPER_CHAR SET name=:power,per_id=:id");
      ...
      if(!$errors){
        ...
        try {
          $stmt = $db->prepare("INSERT INTO FORMS_CHAR SET name=:name, email=:email, year=:byear, pol=:pol, limbs=:limbs, bio=:bio");
          ...
          $pwr=$db->prepare("INSERT INTO SUPER_CHAR SET name=:power, per_id=:id");
          $pwr->bindParam(':id',$id);
